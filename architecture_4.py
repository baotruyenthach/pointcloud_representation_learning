...
class AutoEncoder(nn.Module):
    def __init__(self, normal_channel=False):
        ...
        self.sa4 = PointNetSetAbstraction(npoint=self.num_pts, radius=0.4, nsample=16, in_channel=64 + 3, mlp=[64], group_all=False)
    ...
    def forward(self, xyz):
        ...
        global_feats = x.unsqueeze(-1).repeat(1, 1, self.num_pts)
        ...
        local_feats= self.sa4(l2_xyz, l2_points)[1]
        # print(local_feats.shape, global_feats.shape)
        full_feats = torch.cat((local_feats, global_feats),dim=1)
        # print(full_feats.shape)

        x = F.relu(self.bn1(self.conv1(full_feats)))
