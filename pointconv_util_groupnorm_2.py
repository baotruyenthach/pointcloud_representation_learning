...
class PointConvFeaturePropagation(nn.Module):
    def __init__(self, in_channel, mlp, bandwidth, linear_shape):
        ...
        self.densitynet = DensityNet()
        self.weightnet = WeightNet(3, 16)
        self.linear = nn.Linear(16 * linear_shape, mlp[-1])
        # self.bn_linear = nn.BatchNorm1d(mlp[-1], momentum = 0.5)
        self.bn_linear = nn.GroupNorm(1, mlp[-1])
        
        self.mlp_conv_1 = nn.Conv1d(mlp[0], mlp[0], 1)
        # self.bn_conv_1 = nn.BatchNorm1d(mlp[0], momentum = 0.5)
        self.bn_conv_1 = nn.GroupNorm(1, mlp[0])
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        # if self.group_all:
        new_xyz, new_points, grouped_xyz_norm, grouped_density = sample_and_group_all(xyz1, points1, inverse_density.view(B, N, 1))
        # else:
        # new_xyz, new_points, grouped_xyz_norm, _, grouped_density = sample_and_group(self.npoint, self.nsample, xyz, points, inverse_density.view(B, N, 1))
        # new_xyz, new_points, grouped_xyz_norm, _, grouped_density = sample_and_group(1024, 16, xyz1, points1, inverse_density.view(B, N, 1))
        ...
        grouped_xyz = grouped_xyz_norm.permute(0, 3, 2, 1)
        weights = self.weightnet(grouped_xyz)     
        new_points = torch.matmul(input=new_points.permute(0, 3, 1, 2), other = weights.permute(0, 3, 2, 1)).view(B, 1, -1) ###
        #print(new_points.shape)
        #print(self.linear)
        new_points = self.linear(new_points)
        new_points = self.bn_linear(new_points.permute(0, 2, 1))
        ...
        new_points =  F.relu(self.bn_conv_1(self.mlp_conv_1(new_points)))
